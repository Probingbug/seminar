...
# Initial estimate using template matching
estimated_traj = []
...
for epoch in range(epochs):
    ...
    # Visualization for current epoch
    img_epoch = cv2.cvtColor(img_main, cv2.COLOR_GRAY2BGR)

    # Draw ground truth
    for i in range(len(given_traj) - 1):
        cv2.line(img_epoch, given_traj[i], given_traj[i + 1], (0, 255, 0), 2)

    # Draw estimated
    for i in range(len(estimated_traj) - 1):
        pt1 = tuple(map(int, estimated_traj[i]))
        pt2 = tuple(map(int, estimated_traj[i + 1]))
        cv2.line(img_epoch, pt1, pt2, (0, 0, 255), 2)

    # Show frame
    plt.figure(figsize=(6, 6))
    plt.imshow(img_epoch)
    plt.title(f"Epoch {epoch+1}/{epochs} | RMSE: {rmse:.2f}")
    plt.axis('off')
    plt.pause(0.3)
    plt.close()
...
final_img = cv2.cvtColor(img_main, cv2.COLOR_GRAY2BGR)
for i in range(len(given_traj) - 1):
    cv2.line(final_img, given_traj[i], given_traj[i + 1], (0, 255, 0), 2)
for i in range(len(estimated_traj) - 1):
    pt1 = tuple(map(int, estimated_traj[i]))
    pt2 = tuple(map(int, estimated_traj[i + 1]))
    cv2.line(final_img, pt1, pt2, (0, 0, 255), 2)

plt.figure(figsize=(7, 7))
plt.imshow(final_img)
plt.title(f"Final Trajectory Comparison\nGreen = True | Red = Estimated\nTotal Time: {total_time:.2f} s")
plt.axis('off')
plt.tight_layout()
plt.show()